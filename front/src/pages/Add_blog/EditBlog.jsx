...
class Edit_blog extends Component {
    ...
    async createBlog(params = {}) {
        ...
        let url = `//localhost:8000/blog/create/img?UserId=7655&blogName=${title}&blogtext=${desc}`
        let body = null;
        if (img) {
            body = new FormData();
            body.append(`img`, img);
   ...
   async handlesubmit(ev) {
        ev.preventDefault();
        let { title, desc, img } = this.state
        this.createBlog({ title, desc, img });
         this.getblog();
            ...
            <div className={style.FAform} >
                ...
                <form  onSubmit={this.handlesubmit}>
                  ...
                  <td>
                    ...
                    <td>
                        <img src={`http://localhost:8000/images/${imgG}`}/>
                    </td>
                    </tr>
                    <tr>
                    <label className={style.label1}>
                        <p>decription</p>
                        <input className={style.inp1}  onChange={this.handdleChange} type='text' name='desc' value={desc}></input>
                    </label >
                    </tr>
                    <tr>
                    <label className={style.label1}>
